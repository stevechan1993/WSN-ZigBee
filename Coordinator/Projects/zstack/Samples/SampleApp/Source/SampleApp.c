...
void SampleApp_MessageMSGCB( afIncomingMSGPacket_t *pkt )
{
  ...
  //温湿度传感器
  case SAMPLEAPP_POINT_TO_POINT_CLUSTERID_DHT11:
  ...
  //光线传感器
  case SAMPLEAPP_POINT_TO_POINT_CLUSTERID_LIGHT:
  ...
  else 
  {
    ...
    //HalUARTWrite(0,"No People\n",10);       //串口
    HalLcdWriteString( "No People", HAL_LCD_LINE_3 );//LCD
    rftxInfrad.BUF.value[1] = '0';
  }
  
  
  rftxInfrad.BUF.value[2] = '*';
  
  rftxInfrad.BUF.value[3] = '*';
  rftxInfrad.BUF.value[4] = '*';
  
  rftxInfrad.BUF.value[5] = '*';
  
  osal_memcpy(rftxInfrad.BUF.head,"&&",2);
  osal_memcpy(rftxInfrad.BUF.type,"END",3);
  
  nwk = NLME_GetShortAddr();
  
  To_string(rftxInfrad.BUF.myNWK,(uint8 *)&nwk,2);
  
  nwk = NLME_GetCoordShortAddr();
  
  To_string(rftxInfrad.BUF.pNWK,(uint8 *)&nwk,2);
  
  rftxInfrad.BUF.tail = '*';
  
  HalUARTWrite(0,rftxInfrad.databuf,sizeof(rftxInfrad));
